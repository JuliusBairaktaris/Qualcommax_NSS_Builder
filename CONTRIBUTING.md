...
Thanks for your interest. Two kinds of contribution are particularly valuable:

1. **A new device under `devices/<id>/`.** Fork, build, verify, PR. See [`docs/ADD_A_DEVICE.md`](docs/ADD_A_DEVICE.md).
2. **Patches in `patches/`** that fix upstream regressions or enable a config combination that doesn't work otherwise.
